...
function logger(cookies) {
  let requestTotal = 0;
  let handlingReqTotal = 0;
  debug('log cookies:%j', cookies);
  return function *(next) {
    ...
    function done(event){
      ...
      _.forEach(cookies, function (name) {
        ...
        if (v) {
          cookiesLogs.push(name + ':' + v);
        }
      });
      let str = util.format('%s "%s %s HTTP/%s" %d %d %dms "%s" "%s" "%s" %d-%d-%d', ip, method, url, httpVersion, ctx.status, length || 0, use, ctx.get('referer') || '', ctx.get('user-agent'), cookiesLogs.join(','), handlingReqTotal, index, requestTotal);
      handlingReqTotal--;
      console.info(str);
    }
    yield* next;
